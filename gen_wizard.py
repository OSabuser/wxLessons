...
class MainWizardClass:
    def __init__(self, parent, ini_filename):
        ...
        if not self.database_instance.is_everything_ok:
            wx.MessageBox(f"Не найден файл {ini_filename}!", "Ошибка", wx.ICON_ERROR)
            self.wizard_instance.Destroy()
        else:
            self.pages = []

            self.parent_window = parent  # Ссылка на родительский класс MainPanel(wx.Panel)
            if not isinstance(self.parent_window, wx.Panel):
                raise SystemExit(f"Объекту {self} передан некорректный объект {self.parent_window}")
            ...
            if state:
                ...
                # debug --> print(f"Шифр ПО:  {software_code}")
